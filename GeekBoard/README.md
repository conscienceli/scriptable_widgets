...
| 一天进度条 | — | 全宽细条：灰色轨道是一整天，暖色段是日出到日落，白色游标是此刻。一眼看出「现在在一天的哪个位置」 |
| 天气 | 青 | 天气图标 · 当前温度 / 体感（与实温差 ≥2° 才显示）/ 今日低-高 / 湿度 / UV 当前÷今日峰值 / 降水概率 / AQI / 风向风速 |
| 地理 | 紫 | 经纬度 / 海拔 / 所在区 / 日出 · 日落 |
| AGENDA | 蓝 | **按日期分组**：TODAY / TOMORROW / 之后按星期几，每组一个带色条的小标题，右侧是日期。行数有余量就继续往后铺（`AGENDA_DAYS`，默认 7 天）。今天这组里会插一条**「现在」分隔线**（橙色圆点 + 当前时刻 + 细线），把已过去和还没到的日程分开 |
