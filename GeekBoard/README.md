...
> 预览图为模拟数据，展示的是 430×932 机型（372×364 pt）的**盘后**状态；实机使用 SF Mono 与真实 SF Symbols，观感一致。
...
Scriptable 读不到 Widget 的实际尺寸，只能从屏幕推。**网上流传的旧尺寸表已经不准**——在 430×932 的机器上实测，大号 Widget 是 **372×364 pt**，而旧表写的是 364×382，宽了 8pt（右边留白）、高了 18pt（日程排满时底栏会被挤掉）。

所以这里不查表，用实测标定的比例推算：

```
宽 = 屏宽 × 0.865        高 = 宽 × 0.978
```

| 屏幕 | 机型 | Widget | 主字号 | 左栏行数 | 月历字号 |
|---|---|---|---|---|---|
| 440×956 | 16 Pro Max | 381×373 | 13.5 | 14 | 10.4 |
| 430×932 | 15/16 Pro Max, 15/16 Plus | 372×364 | 13.2 | 14 | 10.1 |
| 402×874 | 16 Pro | 348×340 | 12.3 | 13 | 9.2 |
| 393×852 | 14 Pro, 15, 15 Pro, 16 | 340×333 | 12.0 | 13 | 9.1 |
| 375×812 | X, XS, 11 Pro, 12/13 mini | 324×317 | 11.4 | 13 | 8.5 |
| 320×568 | SE1 | 277×271 | 10.5 | 11 | 7.5 |

右栏三块（月历 / 行情 / 活动）要一起塞进同一份高度，行情和活动的行数是定的，所以**富余量折算成月历字号**——空间紧就把月历缩小，而不是撑爆。实在塞不下（比如 SE1）才减行情行数，最少保留 2 行。左栏另外少排一行做安全余量：估高了会把底栏挤出去，估矮了只是少显示一条未来日程。
...
| 月历的数字非常小甚至看不清 | v1.0 的 bug：日历格子设了固定高度，Scriptable 会把文字硬压进去，连最小缩放下限都不认。已改成只固定宽度 |
